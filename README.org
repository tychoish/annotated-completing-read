...
Scope is controlled by the `scope' variable at the top of the source block:
  'exported   -- all public symbols (name contains no package-private --)
  'autoloaded -- only ;;;###autoload-annotated public symbols
...
#+BEGIN_SRC emacs-lisp :results output raw :exports none
  ...
  (let* ((org-dir (file-name-directory (buffer-file-name)))
         ...
         ;; 'exported | 'autoloaded | (list of symbol name strings)
         (scope 'exported))
    ...
    (defun acr-doc--format-sym (name &optional heading)
      (let* ((sym (intern name))
             (fn-p (fboundp sym))
    ...
    (cond
     ;; Section-based collection for 'exported and 'autoloaded.
     ;; Sections are delimited by long ;;;;... ruler lines; the first ;;
     ;; comment after each ruler names the section.
     ((memq scope '(exported autoloaded))
            ...
            (let ((line (buffer-substring-no-properties
              ...
              (cond
               ...
               ((and current-section
                     ...
                     (not (string-match-p "[a-z]--" (match-string 1 line)))
                     (or (eq scope 'exported)
                         (and (eq scope 'autoloaded) autoload-next)))
