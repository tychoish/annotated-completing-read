...
[[https://melpa.org/#/annotated-completing-read][file:https://melpa.org/packages/annotated-completing-read-badge.svg]]
...
To persist history across Emacs sessions using the built-in =savehist= and =desktop= libraries, call the history setup function in your configuration:

#+BEGIN_SRC emacs-lisp
  (annotated-completing-read-setup-history)
#+END_SRC

For =use-package= users, call it inside the =:config= block:
