...
#+BEGIN_SRC emacs-lisp :results output raw :exports none
  ...
  (let* ((org-dir (file-name-directory (buffer-file-name)))
    ...
    (defun acr-doc--format-sym (name kind &optional heading)
      (let* ((sym (intern name))
             (fn-p (fboundp sym))
        ...
        (format "%s~%s~%s\n\n%s\n\n"
                ...
                name
                (acr-doc--kind-tag sym kind)
                (if doc (acr-doc--format-doc doc) "*no docstring*"))))

    (defun acr-doc--include-p (name kind autoload-p)
    ...
    (cond
                 ...
                 (cond
                  ...
                  ((string-match "^(\\(?:cl-\\)?\\(def[^ ]+\\) +'?\\([^ ()]+\\)" line)
                   (let* ((def-kw (match-string 1 line))
                          ...
                          (kind (acr-doc--defkind def-kw)))
                     (when (and current-section
                                (acr-doc--include-p sym-name kind autoload-next))
         ...
         (lambda (section)
           (let ((syms (if (eq scope 'interactive)
             ...
             (when syms
               (princ (format "** %s\n\n" (car section)))
               (seq-do (lambda (entry)
                         (princ (acr-doc--format-sym (car entry) (cadr entry))))
                       syms))))
         (nreverse sections))))
     ...
     ((listp scope)
      (seq-do (lambda (name)
                (princ (acr-doc--format-sym name 'function "** ")))
              scope))))
#+END_SRC
